...
// The 5x7 ASCII atlas. 95 glyphs, 0x20..0x7E, seven rows top to bottom, five
// bits per row with bit 4 the leftmost pixel — so a glyph is 7 bytes and the
// whole font is 665, baked into the binary with nothing to load at runtime.
//
// Hand-set pixels rather than a rasterised system font: this overlay lives on
// the LOGICAL pane (424x240 for a typical game), which the layer blows up with
// a nearest filter, so an anti-aliased outline would arrive on screen as a
// blur. A pixel grid stays exact at every integer scale. What it replaces was
// seven-segment digits with every letter drawn as an empty box — "iter" and
// "dive" were four boxes each.
static const int kGlyphW = 5, kGlyphH = 7;   // the inked cell
static const int kAdvance = 6, kLineH = 8;   // + one pixel of tracking/leading
static const char kFontFirst = 0x20, kFontLast = 0x7E;

static const uint8_t kFont5x7[95][7] = {
  ...
  { 0x0E, 0x11, 0x01, 0x0D, 0x15, 0x15, 0x0C },   // @
...
/// One glyph at [x,y] (its top-left), each atlas pixel an s-by-s block.
void GpTextOverlay::draw_glyph(int64_t x, int64_t y, unsigned char c,
                               uint8_t r, uint8_t g, uint8_t b, int s) {
  if (c < (unsigned char)kFontFirst || c > (unsigned char)kFontLast) {
    // Outside the atlas: a hollow box, so an unmappable byte is visible rather
    // than a silent gap in the middle of a sentence.
    fill_px(x, y, kGlyphW * s, s, r, g, b);
    fill_px(x, y + (kGlyphH - 1) * s, kGlyphW * s, s, r, g, b);
    fill_px(x, y, s, kGlyphH * s, r, g, b);
    fill_px(x + (kGlyphW - 1) * s, y, s, kGlyphH * s, r, g, b);
    return;
  }
  const uint8_t* rows = kFont5x7[c - (unsigned char)kFontFirst];
  for (int gy = 0; gy < kGlyphH; gy++) {
    uint8_t bits = rows[gy];
    if (bits == 0) continue;
    for (int gx = 0; gx < kGlyphW; gx++) {
      if (bits & (1 << (kGlyphW - 1 - gx))) {          // bit 4 = leftmost
        if (s == 1) set_px(x + gx, y + gy, r, g, b);
        else fill_px(x + gx * s, y + gy * s, s, s, r, g, b);
      }
    }
  }
}
...
void GpTextOverlay::draw_text(int64_t x, int64_t y, const char* text,
  ...
  if (scale < 1) scale = 1;
  if (scale > 16) scale = 16;              // a typo must not paint the world
  int64_t left = x;
  for (const char* p = text; *p != '\0'; p++) {
    unsigned char c = (unsigned char)*p;
    if (c == '\n') { x = left; y += kLineH * scale; continue; }
    if (c == '\r') continue;
    if (c == '\t') { x += 4 * kAdvance * scale; continue; }
    if (c != ' ') draw_glyph(x, y, c, r, g, b, scale);
    x += kAdvance * scale;
  }
  dirty_ = true;
}
